...
router.get("/params", auth, async (req, res) => {
  ...
  let result = await selectQuery(
    `EXEC ProcessAPI.GetOfficialCheckEduFundParams ${MemberID}`
  );
...
router.post("/search", auth, async (req, res) => {
  ...
  let result = await selectQuery(
    `EXEC ProcessAPI.SearchOfficialCheckEduFunds ${MemberID}, N'${JSON.stringify(
      req.body
    )}'`
  );
...
router.post("/response", auth, async (req, res) => {
  ...
  let result = await selectQuery(
    `EXEC ProcessAPI.SaveEduFundOfficialResponse ${MemberID}, N'${JSON.stringify(
      req.body
    )}'`
  );
...
router.post("/report", auth, async (req, res) => {
  ...
  let result = await selectQuery(
    `EXEC ProcessAPI.SaveEduFundOfficialReport ${MemberID}, N'${JSON.stringify(
      req.body
    )}'`
  );
...
router.delete("/report/:recordID", auth, async (req, res) => {
  ...
  let result = await selectQuery(
    `EXEC ProcessAPI.DeleteEduFundOfficialReport ${MemberID}, ${req.params.recordID}`
  );
  ...
  const baseDir = `./uploaded-files/edu-fund-report-files`;
  let dir = "";
