...
router.get("/params", auth, async (req, res) => {
  ...
  let result = await selectQuery(
    `EXEC ProcessAPI.GetFinancialCheckoutParams ${MemberID}`
  );
...
router.post("/search", auth, async (req, res) => {
  ...
  let result = await selectQuery(
    `EXEC ProcessAPI.SearchFinancialCheckouts ${MemberID}, N'${JSON.stringify(
      req.body
    )}'`
  );
...
router.post("/response", auth, async (req, res) => {
  ...
  let result = await selectQuery(
    `EXEC ProcessAPI.SaveFinancialCheckoutResponse ${MemberID}, N'${JSON.stringify(
      req.body
    )}'`
  );
