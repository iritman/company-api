...
router.get("/params", auth, async (req, res) => {
  ...
  let result = await selectQuery(
    `EXEC ProcessAPI.GetInformaticCheckoutParams ${MemberID}`
  );
...
router.post("/search", auth, async (req, res) => {
  ...
  let result = await selectQuery(
    `EXEC ProcessAPI.SearchInformaticCheckouts ${MemberID}, N'${JSON.stringify(
      req.body
    )}'`
  );
...
router.post("/response", auth, async (req, res) => {
  ...
  let result = await selectQuery(
    `EXEC ProcessAPI.SaveInformaticCheckoutResponse ${MemberID}, N'${JSON.stringify(
      req.body
    )}'`
  );
