...
router.get("/params", auth, async (req, res) => {
  ...
  let result = await selectQuery(
    `EXEC ProcessAPI.GetStoreCheckoutParams ${MemberID}`
  );
...
router.post("/search", auth, async (req, res) => {
  ...
  let result = await selectQuery(
    `EXEC ProcessAPI.SearchStoreCheckouts ${MemberID}, N'${JSON.stringify(
      req.body
    )}'`
  );
...
router.post("/response", auth, async (req, res) => {
  ...
  let result = await selectQuery(
    `EXEC ProcessAPI.SaveStoreCheckoutResponse ${MemberID}, N'${JSON.stringify(
      req.body
    )}'`
  );
