...
router.get("/params", auth, async (req, res) => {
  ...
  let result = await selectQuery(
    `EXEC ProcessAPI.GetStorePersonalTransferParams ${MemberID}`
  );
...
router.post("/search", auth, async (req, res) => {
  ...
  let result = await selectQuery(
    `EXEC ProcessAPI.SearchStorePersonalTransfers ${MemberID}, N'${JSON.stringify(
      req.body
    )}'`
  );
...
router.post("/response", auth, async (req, res) => {
  ...
  let result = await selectQuery(
    `EXEC ProcessAPI.SaveStorePersonalTransferResponse ${MemberID}, N'${JSON.stringify(
      req.body
    )}'`
  );
